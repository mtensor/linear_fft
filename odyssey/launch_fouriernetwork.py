...
complexsizes = [32, 64, 128, 256]
optimizer_params = [0.001, 0.01]
L1_betas = [0.01, 0.05, 0.005, 0.1]
        ...
        for beta in L1_betas:
            for ws in weightscales:
                for roff in rseed_offsets:
                    ...
                    fo.write("-rseed %d -rseed_offset %d -weightscale %g -complexsize %d -beta %g -optimizer %g -epochs 10000000 -savefile %s\n" % (rseed, roff, ws, n, beta, optimizer, savefile))
                    i = i+1
                    #what is lr?
                    #epoch thing may need to be cut
fo.close()

call("python run_odyssey_array.py -cmd run_fouriernetwork_odyssey.py -expt %d -cores 8 -hours 40 -mem 24000 -partition general -paramfile %s" % (expt,param_fn), shell=True)
